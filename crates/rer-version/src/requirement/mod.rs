...
fn test_requierement() {
    let a = Requirement::new("voodoo-1");
...
impl Requirements {
    ...
    pub fn from_str(requirements: Vec<&str>) -> Self {
        let requirements = requirements.iter().map(|x| Requirement::new(x)).collect();
