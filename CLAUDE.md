...
- **`rer-python`** — PyO3 bridge. Crate lib name is `rer_solver`; exposes `solve(requests, packages, ...)` where `packages` is the repository as JSON, returning a `SolveResult`. Built into wheels by maturin (`crates/rer-python/pyproject.toml`, no root `pyproject.toml`).
