"""Differential test: ``rer`` vs recorded expectations.
...
This exercises the Python bridge (``rer.solve``) end to end. The
...
If ``rer`` is not installed the whole module is skipped.
"""
...
rer = pytest.importorskip(
    "rer", reason="rer not built (run maturin develop)"
)
...
def _repo_json(packages: dict) -> str:
    ...
    ``rer.solve`` expects: ``{name: {version: {requires, variants}}}``."""
    return json.dumps(
        {
            name: {
...
def test_rer_solver(case: dict):
    ...
    result = rer.solve(case["requests"], _repo_json(case.get("packages", {})))
